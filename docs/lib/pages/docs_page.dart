...
import 'package:docs/pages/docs/sidebar_nav.dart';

const double breakpointWidth = 768;
const double breakpointWidth2 = 1024;

extension CustomWidgetExtension on Widget {
  Widget anchored(OnThisPage onThisPage) => PageItemWidget(onThisPage: onThisPage, child: this);
}
...
class PageItemWidget extends StatelessWidget {
  ...
  const PageItemWidget({super.key, required this.onThisPage, required this.child});
  ...
  Widget buildBadge(BuildContext context) {
    ...
    switch (this) {
      case ShadcnFeatureTag.newFeature:
        copy = theme.copyWith(colorScheme: theme.colorScheme.copyWith(primary: Colors.green));
        badgeText = 'New';
        break;
      case ShadcnFeatureTag.updated:
        copy = theme.copyWith(colorScheme: theme.colorScheme.copyWith(primary: Colors.blue));
        badgeText = 'Updated';
        break;
      case ShadcnFeatureTag.workInProgress:
        copy = theme.copyWith(colorScheme: theme.colorScheme.copyWith(primary: Colors.orange));
        badgeText = 'WIP';
        break;
      case ShadcnFeatureTag.experimental:
        copy = theme.copyWith(colorScheme: theme.colorScheme.copyWith(primary: Colors.purple));
...
class DocsPageState extends State<DocsPage> {
  static final List<ShadcnDocsSection> sections = [
    ...
    ShadcnDocsSection('Form', [
      ...
      ShadcnDocsPage('Formatted Input', 'formatted_input', ShadcnFeatureTag.newFeature),
  ...
  bool isVisible(OnThisPage onThisPage) => currentlyVisible.isNotEmpty && currentlyVisible[0] == onThisPage;
      ...
      builder: (context, query) async* {
        for (final section in sections) {
          ...
          if (resultItems.isNotEmpty) {
            yield [CommandCategory(title: Text(section.title), children: resultItems)];
          }
        }
      },
    );
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRect(
      child: PageStorage(
        bucket: docsBucket,
        child: Builder(
          builder: (context) => StageContainer(
                        ...
                        MediaQueryVisibility(
                          ...
                          alternateChild: AppBar(
                            ...
                            trailing: [
                              GhostButton(
                                density: ButtonDensity.icon,
                                onPressed: () {
                                  openInNewTab('https://github.com/adhunitech/waveui');
                                },
                                child: FaIcon(
                              ...
                              GhostButton(
                                density: ButtonDensity.icon,
                                onPressed: () {
                                  openInNewTab('https://pub.dev/packages/waveui');
                                },
                                child: ColorFiltered(
                                  // turns into white
                                  colorFilter: ColorFilter.mode(theme.colorScheme.secondaryForeground, BlendMode.srcIn),
                          ...
                          child: MediaQueryVisibility(
                            ...
                            alternateChild: _buildAppBar(
                              padding.copyWith(top: 12, bottom: 12, right: 32) * theme.scaling,
                              theme,
                            ),
                            child: _buildAppBar(padding.copyWith(top: 12, bottom: 12) * theme.scaling, theme),
                          ),
                        ),
                        const Divider(),
                      ],
                ...
                child: Row(
                  ...
                  children: [
                                  ...
                                  return SingleChildScrollView(
                                    ...
                                    padding: !hasOnThisPage
                                        ? const EdgeInsets.symmetric(
                                                    horizontal: 40,
                                                    vertical: 32,
                                                  ).copyWith(right: padding.right + 32) *
                                                  theme.scaling +
                                              mq.padding
                                        : const EdgeInsets.symmetric(horizontal: 40, vertical: 32).copyWith(right: 24) *
                                                  theme.scaling +
                                              mq.padding,
                                    child: MediaQuery(
                            ...
                            : Container(
                                ...
                                padding: !hasOnThisPage
                                    ? const EdgeInsets.symmetric(
                                            horizontal: 40,
                                            vertical: 32,
                                          ).copyWith(right: padding.right + 32, bottom: 0) *
                                          theme.scaling
                                    : const EdgeInsets.symmetric(
                                            horizontal: 40,
                                            vertical: 32,
                    ...
                    if (hasOnThisPage)
                      MediaQueryVisibility(
                        minWidth: breakpointWidth2,
                        child: _DocsSecondarySidebar(onThisPage: onThisPage, isVisible: isVisible, padding: padding),
                      ),
                  ],
                ),
              );
  ...
  AppBar _buildAppBar(EdgeInsets padding, ThemeData theme) => AppBar(
    ...
    title: Basic(
      ...
      content: Row(
        ...
        children: [const Text('waveui').textLarge().mono(), Gap(16 * theme.scaling)],
      ),
    ),
    trailing: [
      Align(
...
class _DocsSidebarState extends State<_DocsSidebar> {
  ...
  void initState() {
    super.initState();
    children = [for (var section in widget.sections) _DocsSidebarSection(section: section, pageName: widget.pageName)];
    // do we need didUpdateWidget? nope
    // we don't update the children anyway
  }

  @override
  Widget build(BuildContext context) => SidebarNav(children: children);
}
...
class _DocsSecondarySidebar extends StatefulWidget {
  ...
  const _DocsSecondarySidebar({required this.onThisPage, required this.isVisible, required this.padding});
...
class _DocsSecondarySidebarState extends State<_DocsSecondarySidebar> {
  ...
  void initState() {
    ...
    _sideChildren.add(SidebarSection(header: const Text('On This Page'), children: side));
  }
...
class _DocsSidebarSectionState extends State<_DocsSidebarSection> {
  ...
  void initState() {
    super.initState();
    pages = [for (var page in widget.section.pages) _DocsSidebarButton(page: page, pageName: widget.pageName)];
    // do we need didUpdateWidget? nope
    // we don't update the pages anyway
  }

  @override
  Widget build(BuildContext context) => SidebarSection(header: Text(widget.section.title), children: pages);
}
...
class _DocsSidebarButtonState extends State<_DocsSidebarButton> {
  @override
  Widget build(BuildContext context) => DocsNavigationButton(
