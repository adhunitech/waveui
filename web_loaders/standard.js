...
class ShadcnAppConfig {
    ...
    constructor({ backgroundColor, foregroundColor, fontFamily, fontSize, fontWeight, mainAxisAlignment, crossAxisAlignment, loaderWidget, loaderColor, externalScripts, transitionDuration }) {
        ...
        if (this.backgroundColor == null) {
            this.backgroundColor = localStorage.getItem('waveui.background') || '#09090b';
        }
        if (this.foregroundColor == null) {
            this.foregroundColor = localStorage.getItem('waveui.foreground') || '#ffffff';
        }
        if (this.loaderColor == null) {
            this.loaderColor = localStorage.getItem('waveui.primary') || '#3c83f6';
        }
...
class ShadcnAppThemeChangedEvent extends CustomEvent {
    constructor(theme) {
        super('waveui_theme_changed', { detail: theme });
    }
}
...
class ShadcnApp {
    ...
    loadApp() {
        ...
        window.addEventListener('waveui_app_ready', () => this.onAppReady());
        window.addEventListener('waveui_theme_changed', (event) => this.onThemeChanged(event));
    ...
    onThemeChanged(event) {
        ...
        localStorage.setItem('waveui.background', background);
        localStorage.setItem('waveui.foreground', foreground);
        localStorage.setItem('waveui.primary', primary);
    }
}
...
const waveui = new ShadcnApp(new ShadcnAppConfig(waveui_config));
