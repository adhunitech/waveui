...
class _WaveSwitchTitleState extends State<WaveSwitchTitle> with TickerProviderStateMixin {
  ...
  Widget _toggleButtonsWidget(context) {
    ...
    List<Widget> widgetChildren = widget.nameList.map((name) {
      ...
      double bottomPadding = widget.nameList.length == 1 ? tempBottomPadding : tempBottomPadding - 3;
      if (bottomPadding < 0) {
        bottomPadding = 0;
      }

      return Container(
        padding: EdgeInsets.fromLTRB(widget.padding.left, widget.padding.top, widget.padding.right, bottomPadding),
        child: tx,
      );
    }).toList();
    ...
    Decoration indicator = CustomWidthUnderlineTabIndicator(
      ...
      insets: EdgeInsets.only(left: widget.padding.left, right: widget.padding.right),
    ...
    return Container(
      ...
      child: TabBar(
        ...
        labelColor: widget.nameList.length == 1 ? _color : Get.theme.colorScheme.primary,
