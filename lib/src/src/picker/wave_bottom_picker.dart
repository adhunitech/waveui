...
class WaveBottomPickerWidgetState extends State<WaveBottomPickerWidget> with TickerProviderStateMixin {
  ...
  Widget? _buildConfirmWidget() {
    ...
    if (widget.confirm is Widget) {
      ...
      confirmWidget = _buildDefaultConfirm(WaveIntl.of(context).localizedResource.confirm);
    }
  ...
  Widget? _buildCancelWidget() {
    ...
    if (widget.cancel is Widget) {
      ...
      cancelWidget = _buildDefaultCancel(WaveIntl.of(context).localizedResource.cancel);
    }
    return cancelWidget;
  }
  ...
  Widget _buildDefaultCancel(String? string) {
    return Text(
      ...
      style: TextStyle(color: WaveThemeConfigurator.instance.getConfig().commonConfig.colorTextBase, fontSize: 16.0),
      textAlign: TextAlign.right,
    );
  }
