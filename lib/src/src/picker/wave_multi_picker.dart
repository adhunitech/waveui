import 'package:waveui/src/src/picker/base/wave_picker.dart';
...
class WaveMultiDataPicker extends StatefulWidget {
  ...
  WaveMultiDataPicker({
    ...
    themeData = themeData!.merge(WavePickerConfig(
      ...
      itemTextSelectedStyle: WaveTextStyle(color: textSelectedColor, fontSize: textFontSize),
    ));
  }
...
class _WaveMultiDataPickerState extends State<WaveMultiDataPicker> {
  ...
  Widget _configMultiDataPickerWidget() {
    return Container(
        ...
        child: Row(
            mainAxisSize: MainAxisSize.max, children: widget.pickerTitles != null ? _pickersWithTitle() : _pickers()));
  }
...
/// 实现了部分默认逻辑的 Delegate
class WaveDefaultMultiDataPickerDelegate implements WaveMultiDataPickerDelegate {
  ///数据源
  ...
  WaveDefaultMultiDataPickerDelegate(
      {required this.data, this.firstSelectedIndex = 0, this.secondSelectedIndex = 0, this.thirdSelectedIndex = 0}) {
    if (data.isNotEmpty) {
      ...
      for (WaveMultiDataPickerEntity wavePickerItem in data) {
        if (wavePickerItem.children.isNotEmpty) {
          ...
          for (WaveMultiDataPickerEntity wavePickerItem1 in wavePickerItem.children) {
  ...
  int numberOfRowsInComponent(int component) {
    ...
    if (0 == component) {
      ...
      List<WaveMultiDataPickerEntity> secondMap = data[firstSelectedIndex].children;
  ...
  String titleForRowInComponent(int component, int index) {
    if (0 == component) {
      ...
      WaveMultiDataPickerEntity wavePickerItem = data[firstSelectedIndex];
      List<WaveMultiDataPickerEntity> secondList = wavePickerItem.children;
      List<WaveMultiDataPickerEntity> threeList = secondList[secondSelectedIndex].children;
