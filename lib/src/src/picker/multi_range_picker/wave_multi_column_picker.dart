...
import 'package:waveui/src/src/picker/multi_range_picker/wave_multi_column_converter.dart';
import 'package:waveui/src/src/picker/multi_range_picker/wave_multi_column_list.dart';
...
/// [rowIndex] 数据项所在列中的位置
/// [entity] 被点击的数据项
typedef WaveOnEntityTap = void Function(int columnIndex, int rowIndex, WavePickerEntity entity);
...
typedef WaveOnPickerConfirm = void Function(
    Map<String, List<WavePickerEntity>> results, int? firstIndex, int? secondIndex, int? thirdIndex);
...
class WaveMultiColumnPicker extends StatefulWidget {
  ...
  WaveMultiColumnPicker(
    ...
    themeData = themeData!.merge(WaveThemeConfigurator.instance.getConfig().pickerConfig);
  }
...
class _WaveSelectionGroupViewState extends State<WaveMultiColumnPicker> {
  static const WaveMultiRangeSelConverter defaultConverter = WaveMultiRangeSelConverter();
  ...
  @override
  void initState() {
    _normalColor = WaveThemeConfigurator.instance.getConfig().commonConfig.colorTextBase;
  ...
  Widget _listWidget() {
    ...
    if (!WaveUITools.isEmpty(_firstList) && WaveUITools.isEmpty(_secondList) && WaveUITools.isEmpty(_thirdList)) {
      /// 1、仅有一级的情况
      /// 1.1 一级单选 && 没有自定义范围的情况
      widgets.add(WaveMultiColumnListWidget(
          ...
          singleListItemPick: (int listIndex, int index, WavePickerEntity entity) {
  ...
  void _confirmButtonClickEvent() {
    if (widget.onConfirm != null) {
      ...
      Map<String, List<WavePickerEntity>> result =
          defaultConverter.convertPickedData([widget.entity], includeUnlimitSelection: widget.isIncludeUnLimit);
  ...
  void _setFirstIndex(int firstIndex) {
    ...
    if (widget.entity.children.length > _firstIndex!) {
      List<WavePickerEntity> seconds = widget.entity.children[_firstIndex!].children;
