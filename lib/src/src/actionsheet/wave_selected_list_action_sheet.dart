...
import 'package:waveui/src/src/dialog/wave_dialog.dart';
...
typedef WaveItemDeleteCallback<T> = bool Function(int deleteIdx, T deleteEntity);
...
class WaveSelectedListActionSheet<T> {
  ...
  void showWithTargetKey({required GlobalKey bottomWidgetKey}) {
    RenderBox? renderBox;
    RenderObject? renderObject = bottomWidgetKey.currentContext?.findRenderObject();
  ...
  void _innerShow(bool showByKey) {
    ...
    _WaveActionSheetSelectedItemListContentWidget content = _WaveActionSheetSelectedItemListContentWidget<T>(
      itemWidget: this,
      onDismiss: (isClear) {
    ...
    OverlayEntry overlayEntry = OverlayEntry(builder: (context) {
      ...
      return Positioned(
          ...
          bottom: (showByKey)
              ? _bottomKeyOffset
              : (bottomOffset + MediaQuery.of(context).padding.bottom + MediaQuery.of(context).viewInsets.bottom),
          child: SizedBox(
            width: _maxWidth,
            child: content,
          ));
...
class _WaveActionSheetSelectedItemListContentWidget<T> extends StatefulWidget {
  ...
  _WaveActionSheetSelectedItemListContentWidget(
      {required this.itemWidget, this.onDismiss, required this.itemTitleBuilder, this.onItemDelete, this.controller});
  ...
  @override
  State<StatefulWidget> createState() => _WaveActionSheetSelectedItemListState<T>();
}

class _WaveActionSheetSelectedItemListState<T> extends State<_WaveActionSheetSelectedItemListContentWidget<T?>>
  ...
  void _initStartAnimation() {
    ...
    AnimationController alphaAnimationController =
        AnimationController(duration: const Duration(milliseconds: 200), vsync: this);
    widget._alphaAnimationController = alphaAnimationController;
    Animation<double> yAnimation = Tween(begin: 65.0, end: this.getContentHeight()).animate(yAnimationController)
      ..addListener(() {
        setState(() => {});
      })
    ...
    Animation<double> alphaAnimation = Tween(begin: 0.0, end: 0.7).animate(alphaAnimationController)
      ..addListener(() {});
  ...
  void onClearAction() {
    if (widget.itemWidget.onClear == null) {
      ...
      WaveDialogManager.showConfirmDialog(context,
          title: WaveIntl.of(context).localizedResource.confirmClearSelectedList,
  ...
  void onDeleteItemAction(int idx) {
    if (idx >= widget.itemWidget.items.length) {
      debugPrint('idx:$idx out of range of selectedModelList:${widget.itemWidget.items.length}!!!');
      return;
    }
  ...
  Widget build(BuildContext context) {
    // 顶部标题处理
    String title = (widget.itemWidget.title != null && widget.itemWidget.title!.isNotEmpty)
        ? widget.itemWidget.title!
        : WaveIntl.of(context).localizedResource.selectedList;
    TextStyle titleStyle = const TextStyle(
        fontSize: 18, color: Color(0xff222222), fontWeight: FontWeight.w600, decoration: TextDecoration.none);
              ...
              GestureDetector(
                ...
                child: Container(
                    ...
                    child: Column(children: [
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: topWidgetList),
                      const Divider(
                        height: 0.5,
                        indent: 0,
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            if (index < widget.itemWidget.items.length) {
                              var item = widget.itemTitleBuilder(index, widget.itemWidget.items[index]);
                              if (item is String) {
                                content = Text(
                                  item,
                                  style: const TextStyle(
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    children: <Widget>[
                                      ...
                                      Offstage(
                                        offstage: widget.itemWidget.isDeleteButtonHidden,
                                        child: GestureDetector(
                                          ...
                                          child: Container(
                                              ...
                                              child: WaveUITools.getAssetImage(WaveAsset.iconTrashBin)),
                                        ),
                                      ),
                                    ],
                                  ),
