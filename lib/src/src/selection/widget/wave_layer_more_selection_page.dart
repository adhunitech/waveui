...
class WaveLayerMoreSelectionPage extends StatefulWidget {
  ...
  @override
  _WaveLayerMoreSelectionPageState createState() => _WaveLayerMoreSelectionPageState();
}

class _WaveLayerMoreSelectionPageState extends State<WaveLayerMoreSelectionPage> with SingleTickerProviderStateMixin {
          ...
          child: Column(
            ...
            children: <Widget>[
              AppBar(
                ...
                title: Text(
                  WaveIntl.of(context).localizedResource.selectTitle(widget.entityData.title),
                  style: TextStyle(
                      color: WaveThemeConfigurator.instance.getConfig().commonConfig.colorTextBase,
                      fontSize: 16,
          ...
          onTap: () {
            ...
            if (_firstList[index].filterType == WaveSelectionFilterType.radio) {
              ...
              if (!_firstList[index].isSelected) {
                if (!WaveSelectionUtil.checkMaxSelectionCount(_firstList[index])) {
  ...
  Widget _buildRightItem(int index) {
    bool isSingle = (_currentFirstEntity?.children[index].filterType == WaveSelectionFilterType.radio) ||
        (_currentFirstEntity?.children[index].filterType == WaveSelectionFilterType.unLimit);
          ...
          if (_currentFirstEntity != null) {
            if (isSingle) {
              ...
              if (!_currentFirstEntity!.children[index].isSelected) {
                if (!WaveSelectionUtil.checkMaxSelectionCount(_currentFirstEntity!.children[index])) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(WaveIntl.of(context).localizedResource.filterConditionCountLimited)));
                  return;
                }
              }
              _currentFirstEntity!.children[index].isSelected = !_currentFirstEntity!.children[index].isSelected;
            }
        ...
        children: <Widget>[
          ...
          SizedBox(
            ...
            child: entity.isSelected
                ? WaveUITools.getAssetImageWithBandColor(WaveAsset.selectCheckedStatus)
