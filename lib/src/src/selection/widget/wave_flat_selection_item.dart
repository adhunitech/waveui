...
class _WaveFlatMoreSelectionState extends State<WaveFlatMoreSelection> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.selectionEntity.filterType == WaveSelectionFilterType.layer ||
        widget.selectionEntity.filterType == WaveSelectionFilterType.customLayer) {
...
class __FilterCommonTypeWidgetState extends State<_FilterCommonTypeWidget> {
  ...
  Widget build(BuildContext context) {
    try {
      _tagWidth = (widget.parentWidth - _spacing * (widget.preLineTagSize - 1) - _padding * 2) / widget.preLineTagSize;
      //保留小数点后3位
      _tagWidth = double.parse(_tagWidth.toStringAsFixed(4).substring(0, _tagWidth.toStringAsFixed(4).length - 1));
  ...
  List<Widget> _buildSelectionTag() {
    return widget.selectionEntity.currentShowTagByExpanded(isExpanded).map((WaveSelectionEntity data) {
  ...
  Widget _buildSingleTag(WaveSelectionEntity data) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color:
              data.isSelected ? widget.themeData.tagSelectedBackgroundColor : widget.themeData.tagNormalBackgroundColor,
  ...
  void _showDatePicker(WaveSelectionEntity data) {
    ...
    WaveDatePicker.showDatePicker(context,
        ...
        dateFormat: WaveIntl.of(context).localizedResource.dateFormate_yyyy_MMMM_dd, onConfirm: (dateTime, list) {
      if (mounted) {
        setState(() {
          data.parent?.clearSelectedEntity();
          data.isSelected = true;
...
class __MoreRangeWidgetState extends State<_MoreRangeWidget> {
  ...
  void initState() {
    ...
    minController.text =
        (widget.rangeEntity.customMap!['min'] != null) ? widget.rangeEntity.customMap!['min']?.toString() ?? '' : '';
    maxController.text =
        (widget.rangeEntity.customMap!['max'] != null) ? widget.rangeEntity.customMap!['max']?.toString() ?? '' : '';
    ...
    minController.addListener(() {
      ...
      if (minInput.isNotEmpty && maxInput.isNotEmpty) {
        ...
        if (inputMin != null && inputMin >= min && inputMax != null && inputMax <= max && inputMin <= inputMax) {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: <Widget>[
        _buildRangeField(WaveIntl.of(context).localizedResource.minValue, minController, minFocusNode, widget.width,
            widget.themeData),
        const Padding(
        ...
        _buildRangeField(WaveIntl.of(context).localizedResource.maxValue, maxController, maxFocusNode, widget.width,
            widget.themeData),
      ],
    );
  }
  ...
  Widget _buildRangeField(
    ...
    return Container(
      decoration: const BoxDecoration(color: Color(0xFFF9F9F9), borderRadius: BorderRadius.all(Radius.circular(4.0))),
      height: 32,
      width: width,
      child: Center(
...
class FilterLayerTypeWidget extends StatefulWidget {
  ...
  const FilterLayerTypeWidget(
      {super.key, required this.selectionEntity, this.onCustomFloatingLayerClick, required this.themeData});
...
class _FilterLayerTypeWidgetState extends State<FilterLayerTypeWidget> {
        ...
        Padding(
          ...
          child: GestureDetector(
            onTap: () {
              ...
              } else if (widget.selectionEntity.filterType == WaveSelectionFilterType.customLayer) {
                if (widget.onCustomFloatingLayerClick != null) {
                  int entityIndex = -1;
                  if (widget.selectionEntity.parent != null && widget.selectionEntity.parent!.children.isNotEmpty) {
                    entityIndex = widget.selectionEntity.parent!.children.indexOf(widget.selectionEntity);
                  }
            ...
            child: Row(
              ...
              children: <Widget>[
                Expanded(
                  child: Text(isEmptyCondition() ? WaveIntl.of(context).localizedResource.pleaseChoose : getCondition(),
  ...
  String getCondition() {
    String tmp = "";
    //返回所有选中的
    List<WaveSelectionEntity> selectedList = widget.selectionEntity.selectedList();
