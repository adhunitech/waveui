...
import 'package:waveui/src/src/selection/widget/wave_selection_single_list_widget.dart';
...
typedef SingleListItemSelect = void Function(int listIndex, int index, WaveSelectionEntity entity);
...
class _WaveSelectionGroupViewState extends State<WaveListSelectionGroupWidget> {
  ...
  List<Widget> _configWidgets() {
    ...
    // TODO 判断是否添加 Bottom
    if (totalLevel == 1 && widget.entity.filterType == WaveSelectionFilterType.radio) {
    } else {
  ...
  Widget _listWidget() {
    ...
    if (!WaveUITools.isEmpty(_firstList) && WaveUITools.isEmpty(_secondList) && WaveUITools.isEmpty(_thirdList)) {
      ...
      widgets.add(WaveSelectionSingleListWidget(
          ...
          singleListItemSelect: (int listIndex, int index, WaveSelectionEntity entity) {
  ...
  _processSelectedStatus(WaveSelectionEntity entity) {
    if (entity.children.isNotEmpty) {
      ...
      if (entity.hasCheckBoxBrother()) {
        entity.isSelected = entity.children.where((_) => _.isSelected).isNotEmpty;
      }
    }
  }
