...
class Annotation {
  ...
  final RegExp regExp;
  final TextSpan Function({required String text, TextStyle? textStyle}) spanBuilder;
}

class WaveReadMore extends StatefulWidget {
  const WaveReadMore(
    this.data, {
    super.key,
    this.isCollapsed,
    ...
    this.trimLength = 240,
    this.trimLines = 2,
    this.trimMode = WaveTrimMode.Length,
    this.style,
    this.textAlign,
    this.textDirection,
    this.locale,
  ...
  @override
  WaveReadMoreState createState() => WaveReadMoreState();
}
...
class WaveReadMoreState extends State<WaveReadMore> with SingleTickerProviderStateMixin {
  ...
  void _onTap() {
    if (widget.isExpandable) {
      final newValue = !_effectiveIsCollapsed.value;
      _effectiveIsCollapsed.value = newValue;
  ...
  @override
  void didUpdateWidget(WaveReadMore oldWidget) {
  ...
  Widget build(BuildContext context) =>
      ValueListenableBuilder(valueListenable: _effectiveIsCollapsed, builder: _builder);
  ...
  Widget _builder(BuildContext context, bool isCollapsed, Widget? child) {
    ...
    Widget result = LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final text = TextSpan(
          children: [if (preTextSpan != null) preTextSpan, annotatedText, if (postTextSpan != null) postTextSpan],
        );
        ...
        switch (widget.trimMode) {
          case WaveTrimMode.Length:
            if (widget.trimLength < widget.data.length) {
              final effectiveAnnotatedText =
                  isCollapsed
                      ? _trimTextSpan(textSpan: annotatedText, spanStartIndex: 0, endIndex: widget.trimLength).textSpan
                      : annotatedText;
            ...
            break;
          case WaveTrimMode.Line:
            if (textPainter.didExceedMaxLines) {
              final effectiveAnnotatedText =
                  isCollapsed
                      ? _trimTextSpan(textSpan: annotatedText, spanStartIndex: 0, endIndex: endIndex).textSpan
                      : annotatedText;
        ...
        final content = Text.rich(
          TextSpan(children: [if (preTextSpan != null) preTextSpan, textSpan, if (postTextSpan != null) postTextSpan]),
          textAlign: textAlign,
          textDirection: textDirection,
          softWrap: true,
  ...
  _TextSpanTrimResult _trimTextSpan({required TextSpan textSpan, required int spanStartIndex, required int endIndex}) {
    ...
    if (children != null) {
      for (final child in children) {
        if (child is TextSpan) {
          final result = _trimTextSpan(textSpan: child, spanStartIndex: spanEndIndex, endIndex: endIndex);
