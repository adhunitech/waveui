...
import 'package:waveui/src/widgets/src/ui/parts/calendar.dart';
import 'package:waveui/src/widgets/src/ui/parts/during_calendar.dart';
...
abstract class PickerCalendarWidget extends StatefulWidget {
  const PickerCalendarWidget({super.key, required this.arguments, this.multiSelectionMaxDateBuilder});

  final PickerCalendarArgs arguments;
  final MultiSelectionMaxDateBuilder? multiSelectionMaxDateBuilder;
}

abstract class PickerCalendarState<T extends PickerCalendarWidget> extends State<T> {
  final GlobalKey calendarKey = GlobalKey();

  final double keyboardMenuMaxHeight = 48;

  double get keyboardMenuHeight => keyboardMenuMaxHeight * (1 - args.keyboardHeightRatio());
      ...
      return SizedBox(
        child: MultipleCalendarWidget(
          ...
          boxDecoration: BoxDecoration(
            color:
                args.options.backgroundDecoration != null && !isWide
  ...
  Widget picker({required bool isWide}) {
    ...
    for (final x in args.listOptions) {
      ...
      if (separator != null) {
        ...
        if (separatorTypes.contains(x.type)) {
          items.add(
            separator.dateSeparatorBuilder?.call(context, textStyle) ?? Text(separator.date.display, style: textStyle),
          );
        } else if (x.type == DateType.hour || (x.type == DateType.minute && args.options.withSecond)) {
          items.add(
            separator.timeSeparatorBuilder?.call(context, textStyle) ?? Text(separator.time.display, style: textStyle),
          );
        } else if (x.type == lastYmdDateType && args.pickerType == DateTimePickerType.datetime) {
          items.add(
            separator.dateTimeSeparatorBuilder?.call(context, textStyle) ??
                Text(separator.dateTime.display, style: textStyle),
          );
        }
      }
    }
    ...
    if (DateTimePickerType.time == args.pickerType && args.options.useAmpm) {
      final hourOption = args.listOptions.firstWhere((e) => e.type == DateType.hour);
      items.insert(
        0,
        Expanded(
    ...
    return SizedBox(child: Align(alignment: Alignment.topCenter, child: Row(children: items)));
  }
  ...
  Widget keyboardMenu({required bool isWide}) {
    return Container(
      decoration: BoxDecoration(color: args.options.getForegroundColor(context).withValues(alpha: 0.9)),
...
class PickerCalendarWideWidget extends PickerCalendarWidget {
  ...
  @override
  PickerCalendarState<PickerCalendarWideWidget> createState() => _PickerCalendarWideWidgetState();
}

class _PickerCalendarWideWidgetState extends PickerCalendarState<PickerCalendarWideWidget> {
  @override
  Widget build(BuildContext context) {
    ...
    Widget child = Row(
      children: [
        if (args.pickerType != DateTimePickerType.time) ...[_left(), const SizedBox(width: 16)],
        Expanded(child: Column(children: [args.headerBuilder(context), Expanded(child: picker(isWide: true))])),
      ],
    );
    ...
    if (args.options.isTopTitleHeader) {
      height += 40;
      wrap = Column(
        children: [TopTitleWidget(options: args.options, selectedDayNotifier: args.dateState), Expanded(child: child)],
      );
    }

    return Container(
      height: height + args.keyboardHeightRatio() * 172,
      decoration: args.options.backgroundDecoration ?? BoxDecoration(color: args.options.getBackgroundColor(context)),
      padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 8),
      child: SafeArea(top: false, child: wrap),
    );
  }
  ...
  Widget _left() {
    return Container(
      ...
      child: Stack(
        children: [
          Positioned.fill(child: calendar(background: args.options.getForegroundColor(context), isWide: true)),
          Positioned.fill(
            child: Visibility(
...
class PickerCalendarStandardWidget extends PickerCalendarWidget {
  ...
  @override
  PickerCalendarState<PickerCalendarStandardWidget> createState() => _PickerCalendarStandardWidgetState();
}

class _PickerCalendarStandardWidgetState extends PickerCalendarState<PickerCalendarStandardWidget> {
  @override
  Widget build(BuildContext context) {
    return AnimatedBuilder(animation: widget.calendarAnimationController, builder: builder);
  }
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      //top title
                      if (args.options.isTopTitleHeader)
                        TopTitleWidget(options: args.options, selectedDayNotifier: args.dateState),
      ...
      children: [
        Visibility(
          ...
          child: FadeTransition(
            ...
            child: calendar(background: args.options.getBackgroundColor(context), isWide: false),
          ),
        ),
        Visibility(
          visible: widget.calendarAnimation.value != 1,
          child: FadeTransition(opacity: widget.pickerFormAnimation, child: picker(isWide: false)),
        ),
      ],
    );
  }
