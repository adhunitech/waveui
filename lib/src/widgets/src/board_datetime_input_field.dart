...
class BoardDateTimeInputFieldValidators {
  ...
  String? _errorIllegal(String text) {
    return onIllegalFormat?.call(text) ?? BoardDateTimeInputError.illegal.message;
  }

  String? _errorOutOfRange(String text) {
    return onOutOfRange?.call(text) ?? BoardDateTimeInputError.outOfRange.message;
  }
...
///
/// ```
class BoardDateTimeInputField<T extends BoardDateTimeCommonResult> extends StatefulWidget {
  ...
  @override
  State<BoardDateTimeInputField> createState() => _BoardDateTimeInputFieldState<T>();
}

class _BoardDateTimeInputFieldState<T extends BoardDateTimeCommonResult> extends State<BoardDateTimeInputField<T>>
  ...
  void _focusListener() {
    ...
    if (!focusNode.hasFocus) {
      ...
      final existsBdtFocusNode = focusList.any((x) => x is BoardDateTimeInputFocusNode || x is PickerContentsFocusNode);
    ...
    if (val != null) {
      void apply() {
        ...
        if (selectedDate != val) {
          ...
          widget.onResult?.call(BoardDateTimeCommonResult.init(widget.pickerType, val) as T);
        }
      }
  ...
  void initState() {
    ...
    textController = TextEditingController(
      text: initial != null ? DateFormat(format).format(rangeDate(initial)) : null,
    );
    textController.addListener(() {});

    pickerFocusNode = PickerContentsFocusNode(debugLabel: 'Picker Focus Node', skipTraversal: true);
    ...
    return Form(
      ...
      child: TextFormField(
        ...
        decoration:
            widget.decoration ??
            InputDecoration(
              border: OutlineInputBorder(borderRadius: borderRadius, borderSide: const BorderSide(color: Colors.black)),
              focusedBorder: OutlineInputBorder(
                borderRadius: borderRadius,
                borderSide: BorderSide(color: options.activeColor ?? Theme.of(context).primaryColor),
              ),
            ...
            widget.validators.showMessage
                ? (value) {
                  ...
                  final error =
                      validate(
                        value,
                        complete: textController.text.length != textOffset || textOffset == format.length,
                      ).error;
  ...
  void checkFormat(String val, {bool complete = false}) {
    ...
    if (widget.pickerType == DateTimePickerType.time) {
      final hour = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.hour);
      final minute = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.minute);
      final second = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.second);
      ...
      if (complete && options.withSecond && existHour && (second == null || second.text.isEmpty)) {
    ...
    // Corrects for date or datetime if date or datetime
      ...
      final year = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.year);
      final month = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.month);
      final day = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.day);
      if (year != null && year.text.isNotEmpty && complete) {
        if (month == null || month.text.isEmpty) {
          final bloc = TextBloc(text: '01', start: 0, end: 0)..dateType = DateType.month;
        ...
        if (day == null || day.text.isEmpty) {
          final bloc = TextBloc(text: '01', start: 0, end: 0)..dateType = DateType.day;
      ...
      if (month != null && month.text.isNotEmpty && day != null && day.text.isNotEmpty) {
        final year = result.splited!.firstWhereOrNull((e) => e.dateType == DateType.year);

        void setYear() {
          final bloc = TextBloc(text: (widget.initialDate ?? DateTime.now()).year.toString(), start: 0, end: 0);
    ...
    if (complete && widget.pickerType == DateTimePickerType.datetime) {
      if (result.splited!.length <= 5) {
        final diff = 5 - (result.splited!.where((e) => e.text.isNotEmpty).toList().length);
        for (var i = 0; i < diff; i++) {
          ...
          final index = result.splited!.indexWhere((e) => e.dateType == bloc.dateType);
          if (index < 0) {
            result.splited!.add(bloc);
          } else {
    ...
    if (result.splited!.isNotEmpty && result.splited!.last.text.isEmpty && date.length >= format.length) {
    ...
    textController.selection = TextSelection.fromPosition(TextPosition(offset: offset));

    final datetime = result.datetime;
    // Callback only if there are no errors
    if (datetime != null && result.error == null) {
      if (datetime.isAfter(minimumDate) && datetime.isBefore(maximumDate) && selectedDate != datetime) {
        selectedDate = datetime;
        widget.controller?.updateSelectedDate(selectedDate);
        widget.onChanged.call(datetime);
        widget.onResult?.call(BoardDateTimeCommonResult.init(widget.pickerType, datetime) as T);
