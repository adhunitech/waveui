...
class BoardPickerItemOption {
  ...
  factory BoardPickerItemOption.init({required ItemOptionArgs args}) {
    ...
    switch (args.type) {
      ...
      case DateType.month:
        map = minmaxList(args.pickerType, DateType.month, args.date, mi, ma, multiSelection: args.multiSelection);
  ...
  factory BoardPickerItemOption.year({required ItemOptionArgs args}) {
    ...
    final ma = args.maximum ?? DateTime(DateTimeUtil.maximumYear, 12, 31, 23, 59);
  ...
  int get maxLength {
    if (type == DateType.year) {
      ...
      if (monthFormat == PickerMonthFormat.number) {
        ...
        final maxLength = monthMap().values.reduce((a, b) => a.length > b.length ? a : b).length;
        return maxLength;
      } else {
        return 3;
      }
  ...
  int get flex {
    if (type == DateType.year) {
      return 2;
    } else if (type == DateType.month && monthFormat == PickerMonthFormat.long && args.wide()) {
      return 2;
    }
    return 1;
  }
  ...
  void updateAmPm(AmPm ap) {
    ...
    if (entry != null) {
      // 切り替え先の時間が一致する値を取得する
      final nextEntry = next.entries.firstWhereOrNull((e) => e.value.hour == entry.hour);
      if (nextEntry != null) {
  ...
  static Map<int, int> minmaxList(
    ...
    switch (dt) {
      case DateType.year:
        ...
        return !multiSelection && currentMap != null ? currentMap : createMap(minimum.year, maximum.year);
      case DateType.month:
        int minMonth = 1;
        int maxMonth = 12;
  ...
  DateTime calcDate(DateTime date, {int? newDay}) {
    switch (type) {
      ...
      case DateType.second:
        return DateTime(date.year, date.month, date.day, date.hour, date.minute, itemMap[selectedIndex]!);
    }
  }
  ...
  Map<int, String> monthMap() {
    // DateFormat
    final dateFormat =
        monthFormat == PickerMonthFormat.long && args.wide() ? DateFormat.MMMM(locale) : DateFormat.MMM(locale);
