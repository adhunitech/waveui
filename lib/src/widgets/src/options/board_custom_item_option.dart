...
class BoardPickerCustomItemOption extends BoardPickerItemOption {
  ...
  factory BoardPickerCustomItemOption.init({required ItemOptionArgs args}) {
    ...
    if (args.type == DateType.year) {
      final list = args.customList!.where((x) => x >= mi.year && x <= ma.year).toList();
      for (var i = 0; i < list.length; i++) {
        map[i] = list[i];
      }
    } else {
      final m = BoardPickerItemOption.minmaxList(args.pickerType, args.type, args.date, mi, ma);
    ...
    switch (args.type) {
      case DateType.year:
        ...
        selected = BoardPickerItemOption.indexFromValue(max(args.date.year, minY), map);
      case DateType.month:
  ...
  void updateList(DateTime date) {
    ...
    final tmp = value;
    // Generate new maps
    final m = BoardPickerItemOption.minmaxList(pickerType, type, date, minimumDate, maximumDate);
    final values = m.values.toList();
