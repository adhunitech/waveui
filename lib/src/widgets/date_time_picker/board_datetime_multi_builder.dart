...
class MultiBoardDateTimeContent<T extends BoardDateTimeCommonResult> extends BoardDateTimeContent<T> {
  ...
  @override
  State<MultiBoardDateTimeContent> createState() => _MultiBoardDateTimeContentState<T>();
}
...
class _MultiBoardDateTimeContentState<T extends BoardDateTimeCommonResult>
  ...
  @override
  DateTime get currentDate => currentDateType.value == MultiCurrentDateType.start ? startDate.value : endDate.value;
  ...
  DateTime? get maximumDate {
    if (widget.pickerType == DateTimePickerType.time) {
      ...
      if (currentDateType.value == MultiCurrentDateType.start) {
        return DateTime(now.year, now.month, now.day, endDate.value.hour, endDate.value.minute, endDate.value.second);
      }
      return DateTime(
        now.year,
        now.month,
      ...
      final builtMaxDate = widget.multiSelectionMaxDateBuilder?.call(startDate.value);
  ...
  void _setFocusNode(bool byPicker) {
    if (byPicker && widget.pickerFocusNode != null) {
      ...
      if (!fn.hasFocus && FocusManager.instance.primaryFocus! is! BoardDateTimeInputFocusNode) {
        fn.requestFocus();
      }
    }
  }
  ...
  void changeMultiDate(DateTime val, MultiCurrentDateType type, {bool byPicker = false}) {
  ...
  void notify() {
    for (var element in itemOptions) {
      element.updateList(currentDateType.value == MultiCurrentDateType.start ? startDate.value : endDate.value);
    }
  ...
  void reset() {
    ...
    changeDateTime(currentDateType.value == MultiCurrentDateType.start ? startDate.value : endDate.value);
  ...
  void notifyInitialValue() {
    ...
    if (!changedDate) {
      widget.onUpdateByClose?.call(rangeDate(startDate.value), rangeDate(endDate.value));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return AnimatedBuilder(animation: openAnimationController, builder: _openAnimationBuilder);
      },
    );
  }

  Widget _openAnimationBuilder(BuildContext context, Widget? child) {
    final animation = openAnimationController.drive(curve).drive(Tween<double>(begin: 0.0, end: 1.0));
  ...
  void onChangeDateType(MultiCurrentDateType val) {
    ...
    setState(() {
      setupOptions(
        currentDateType.value == MultiCurrentDateType.start ? startDate.value : endDate.value,
        widget.pickerType,
      );
    });
  }
