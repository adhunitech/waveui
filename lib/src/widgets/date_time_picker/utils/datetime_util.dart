...
class DateTimeUtil {
  ...
  static int? getExistsMaxDate(List<BoardPickerItemOption> options, BoardPickerItemOption selected, int val) {
  ...
  static DateTime rangeDate(DateTime date, DateTime? minimumDate, DateTime? maximumDate) {
  ...
  static Map<int, AmpmCotrast> ampmContrastMap = {...ampmContrastAmMap, ...ampmContrastPmMap};
...
class AmpmCotrast {
  ...
  factory AmpmCotrast.am(int hour, int index) {
    return AmpmCotrast(ampm: AmPm.am, hour: hour, index: index);
  }

  factory AmpmCotrast.pm(int hour, int index) {
    return AmpmCotrast(ampm: AmPm.pm, hour: hour, index: index);
  }
}
...
extension DateTimeExtension on DateTime {
  ...
  bool isMinimum(DateTime date, DateType dt, {bool equal = true}) {
    ...
    switch (dt) {
      ...
      case DateType.day:
        ...
        return year <= date.year && month <= date.month && operator(day, date.day);
      case DateType.hour:
        if (year < date.year) return true;
        if (year <= date.year && month < date.month) return true;
        ...
        return year <= date.year && month <= date.month && day <= date.day && operator(hour, date.hour);
  ...
  bool isWithinRangeAndEqualsDate(DateTime minimum, DateTime maximum) {
    ...
    if (minimum.year == year && minimum.month == month && minimum.day == day) {
      return true;
    } else if (maximum.year == year && maximum.month == month && maximum.day == day) {
      return true;
    }
    return false;
  }
