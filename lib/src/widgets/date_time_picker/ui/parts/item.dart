...
class ItemWidgetState extends State<ItemWidget> with SingleTickerProviderStateMixin {
  ...
  bool get useAmpmMode {
    return widget.option.type == DateType.hour && widget.option.useAmpm && widget.option.ampm != null;
  }
  ...
  void setMap({Map<int, int>? newMap}) {
    if (useAmpmMode) {
      ...
      final mapEntry =
          DateTimeUtil.ampmContrastMap.entries.where((x) {
            return values.contains(x.key) && x.value.ampm == widget.option.ampm;
          }).toList();
      map = {for (var i = 0; i < mapEntry.length; i++) i: mapEntry[i].value.hour};
    } else {
  ...
  int getWheelIndex(int index) {
    if (useAmpmMode) {
      ...
      final i = map.entries.firstWhereOrNull((x) => x.value == cotrast.hour)?.key;
  ...
  void callbackOnChange(int index) {
    if (useAmpmMode) {
      ...
      final mapIndex = contrastMap.entries.firstWhereOrNull((e) => e.value.hour == hour);
      if (mapIndex != null) {
        final idx = widget.option.itemMap.entries.firstWhereOrNull((e) => e.value == mapIndex.key);
        if (idx != null) {
  ...
  void initState() {
    ...
    textController = TextEditingController(text: widget.option.isMonthText ? monthMap[day] : '$day');
    textController.selection = TextSelection.fromPosition(TextPosition(offset: textController.text.length));

    widget.option.focusNode.addListener(focusListener);

    correctAnimationController = AnimationController(vsync: this, duration: const Duration(milliseconds: 300));
  ...
  void toAnimateChange(int index, {bool button = false}) {
    ...
    scrollController.animateToItem(index, duration: duration, curve: Curves.easeIn);
  }
  ...
  void updateState(Map<int, int> newMap, int newIndex) {
    ...
    Future.delayed(const Duration(milliseconds: 10)).then((_) {
      if (needAnimation) {
        scrollController.animateToItem(selectedIndex, duration: duration, curve: Curves.easeIn);
      } else {
                  ...
                  child: AnimatedBuilder(
                    ...
                    builder: (context, child) {
                      return Material(
                        ...
                        child: SizedBox(height: itemSize, width: double.infinity),
                      );
                    },
                  ),
                ),
                        ...
                        child: GestureDetector(
                          child: Focus(
                            ...
                            child: ListWheelScrollView.useDelegate(
                              ...
                              childDelegate: ListWheelChildListDelegate(children: [for (final i in map.keys) _item(i)]),
                            ),
                          ),
                          onTapDown: (details) {
                            _updateFocusNode();
                          ...
                          onTapUp: (details) {
                            ...
                            if (widget.showedKeyboard()) {
                              clickOffset = details.localPosition.dy - (itemSize * 3 / 2);
                            } else {
                              clickOffset = details.localPosition.dy - (itemSize * wheelCount / 2);
                            }
                ...
                Visibility(
                  ...
                  child: _centerAlign(
                    TextField(
                      ...
                      keyboardType: widget.option.isMonthText ? TextInputType.text : TextInputType.number,
    ...
    try {
      ...
      if (widget.option.isMonthText) {
        ...
        try {
          ...
          data = monthMap.entries.firstWhereOrNull((e) => e.value.toLowerCase() == text.toLowerCase())!.key;
        }
      } else {
        data = int.parse(text);
      }
  ...
  void changeText(String text, {bool toDefault = false}) {
    ...
    if (index == null) {
      index = selectedIndex;
      final day = map[index]!;
      textController.text = widget.option.isMonthText ? monthMap[day]! : day.toString();
    }
  ...
  void onChangeText(String text) {
    ...
    // Animated wheel movement
    wheelDebouceTimer = Timer(const Duration(milliseconds: 200), () {
      wheelDebouceTimer?.cancel();
  ...
  Widget _centerAlign(Widget child) {
    return Align(alignment: Alignment.center, child: SizedBox(height: itemSize, width: double.infinity, child: child));
  }
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    try {
      ...
      if (!list.contains(value)) {
        final x = list.where((x) => x.toString().contains(newValue.text));
        if (x.isEmpty) {
          return oldValue;
        }
      }
...
class AmpmItemWidgetState extends State<AmpmItemWidget> {
  ...
  Widget _item(int i) {
    ...
    return Center(child: Text(map[i]!.display, style: textStyle));
  }
}
