...
class SingleCalendarWidget extends CalendarWidget {
  ...
  @override
  CalendarWidgetState<SingleCalendarWidget> createState() => _SingleCalendarWidgetState();
}

class _SingleCalendarWidgetState extends CalendarWidgetState<SingleCalendarWidget> {
  @override
  void initState() {
...
abstract class CalendarWidgetState<T extends CalendarWidget> extends State<T> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [Positioned.fill(child: _calendar()), Positioned(top: 0, left: 0, right: 0, child: _moveButtons())],
    );
  }
      ...
      child: Row(
        children: [
          Text(
            ...
            style: Theme.of(
              context,
            ).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800, color: widget.textColor),
          ),
  ...
  Widget _weekdays() {
    List<String> weekdays = DateFormat.EEEE(widget.languages.locale).dateSymbols.SHORTWEEKDAYS;
          ...
          for (var i = 0; i < weekdays.length; i++)
            Expanded(
              child: Center(
                child: Text(
                  weekdays[i],
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(color: textColor(weekdayVals[i], false)),
                ),
              ),
            ),
        ],
  ...
  List<Widget> _generateCalendarOfMonth(DateTime date) {
    ...
    if (date.year == minimumDate.year && date.month == minimumDate.month) {
      diabledList.addAll([for (var i = 1; i < minimumDate.day; i++) i]);
    } else if (date.isMinimum(minimumDate, DateType.month)) {
      diabledList.addAll([for (var i = 1; i <= y.day; i++) i]);
    }

    // max date
    if (date.year == maximumDate.year && date.month == maximumDate.month) {
      diabledList.addAll([for (var i = maximumDate.day + 1; i <= y.day; i++) i]);
    }
  ...
  Widget _monthItem(DateTime first, int i, bool disabled) {
    ...
    final textStyle = Theme.of(
      context,
    ).textTheme.bodyLarge?.copyWith(color: selected ? widget.activeTextColor : textColor(z.weekday, disabled));
    Widget child = Text('$i', style: textStyle);
    if (selected) {
      final built = widget.calendarSelectionBuilder?.call(context, '$i', textStyle);
      if (built != null) {
        child = built;
      }
    }
...
class CalendarSelectedProps {
  ...
  CalendarSelectedProps({required this.margin, required this.borderRadius});
}
