...
class MultipleCalendarWidget extends CalendarWidget {
  ...
  @override
  CalendarWidgetState<MultipleCalendarWidget> createState() => _MultipleCalendarWidgetState();
}

class _MultipleCalendarWidgetState extends CalendarWidgetState<MultipleCalendarWidget> {
  ...
  DateTime get maximumDate {
    if (firstTouched) {
      return widget.multiSelectionMaxDateBuilder?.call(selectedDate.first) ?? widget.maximumDate;
    }
  ...
  void initState() {
    ...
    selectedDate = [widget.startDate.value, widget.endDate.value];
  }
  ...
  void onTap(DateTime date) {
    ...
    if (firstTouched) {
      ...
      firstTouched = true;
      widget.onChange(to, DateTime(to.year, to.month, to.day, initialEndDate.hour, initialEndDate.minute));
      selectedDate = [to];
  ...
  CalendarSelectedProps getProps(DateTime date) {
    ...
    if (result) {
      if (first.compareDate(last)) {
        return CalendarSelectedProps(margin: const EdgeInsets.all(space), borderRadius: BorderRadius.circular(radius));
      }
      if (date.compareDate(first)) {
        ...
        return CalendarSelectedProps(
          margin: const EdgeInsets.only(right: space, top: space, bottom: space),
