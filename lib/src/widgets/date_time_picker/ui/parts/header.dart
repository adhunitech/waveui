...
class BoardDateTimeHeaderState extends State<BoardDateTimeHeader> {
  ...
  Widget build(BuildContext context) {
    ...
    final closeButton = closeButtonBuilder(context, widget.modal, widget.onClose);
  ...
  List<Widget> _dateItems(BuildContext context) {
    ...
    return [_calendarButton(), Wrap(spacing: widget.wide ? 20 : 12, children: buttons)];
  }
  ...
  List<Widget> _timeItems(BuildContext context) {
    ...
    return [
      ...
      _textButton(context, widget.languages.now, () => widget.onChangTime(DateTime.now())),
    ];
  }
    ...
    final child = Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: _title()))),
          ..._rightButton(),
        ],
      ),
    );
    ...
    if (widget.options.boardTitleBuilder != null) {
      ...
      final Widget titleWidget = widget.options.boardTitleBuilder!(
        ...
        widget.options.boardTitleTextStyle ??
            Theme.of(
              context,
            ).textTheme.titleMedium?.copyWith(color: widget.options.getTextColor(context), fontWeight: FontWeight.bold),
        widget.dateState.value,
      );

      return FittedBox(child: titleWidget);
    }
...
class TopTitleWidget extends StatelessWidget {
  const TopTitleWidget({super.key, required this.options, required this.selectedDayNotifier});
  ...
  Widget _getTitle(BuildContext context, TextStyle? textStyle) {
    if (options.boardTitleBuilder != null) {
      return options.boardTitleBuilder!(context, textStyle, selectedDayNotifier.value);
    }

    if (options.boardTitle != null) {
      return Text(options.boardTitle.toString(), style: textStyle, maxLines: 1);
    }
    ...
    return Container(
      ...
      child: _getTitle(
        ...
        options.boardTitleTextStyle ??
            Theme.of(
              context,
            ).textTheme.titleMedium?.copyWith(color: options.getTextColor(context), fontWeight: FontWeight.bold),
      ),
    );
  }
}
